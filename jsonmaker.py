...
class Postcard(db.Model):
    ...
    def to_dict(self, new_id=None, dance_number_text=None):
        # 날짜+시간 포맷: "2024년 05월 12일 14시30분15초"
...
def generate_json_file_with_sequential_ids():
    try:
        # timestamp 오름차순 정렬
        postcards = Postcard.query.order_by(Postcard.timestamp).all()

        data = {
            "postcards": [
                postcard.to_dict(
                    new_id=index + 1, dance_number_text=f"{index+1}번째 춤"
                )
        ...
        print("✅ allData.json 생성 완료:", output_path)
